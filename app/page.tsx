...
// tree traversal, binary tree, remove,insert button and traversal buttons working fine
...
export default function Page() {
  ...
  const handleRemove = () => {
    if (inputValue === null) return;
    setTree(removeNode(tree, inputValue));
    setInputValue(null);
    setTraversalResult([]);
  };
  ...
  const removeNode = (
    ...
    if (value < root.value) {
      root.left = removeNode(root.left ?? null, value);
    } else if (value > root.value) {
      root.right = removeNode(root.right ?? null, value);
    } else {
  ...
  const minValue = (node: TreeNode): number => {
    let current = node;
    while (current.left) {
